...
class TestMultiChannelCadence(IntegrationTestCase):
    ...
    def test_process_cadence_step_sift_payload_markdown(self, mock_post, mock_wait):
        from unittest.mock import MagicMock
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_post.return_value = mock_response

        # Setup Sender User with HTML Bio
        ...
        if not frappe.db.exists("User", user_email):
            frappe.get_doc({
                ...
                "first_name": "Test",
                "last_name": "Sender",
                "bio": "<b>Bold Bio</b> with <i>italic</i>",
                "send_welcome_email": 0
            }).insert(ignore_permissions=True)

        # Ensure the test lead exists (already created in setUpClass)
        # We also need a Prompt Template
        template_name = "Test Prompt Template"
        if not frappe.db.exists("Email Template", template_name):
            template = frappe.get_doc({
                ...
                "user_prompt": "Write an email to the lead"
            }).insert(ignore_permissions=True)
        else:
        ...
        history = frappe.get_doc({
            ...
            "reference_name": self.lead_name,
            "content": "<h1>Previous History</h1><p>Some notes</p>",
        ...
        cadence.db_set("owner", user_email)
        
        # Setup Schedule
        schedule = frappe.get_doc({
            "doctype": "Cadence Multi Channel Schedule",
            "parent": cadence.name,
            "parenttype": "Multi Channel Cadence",
        ...
        original_url = sift_settings.sift_base_url
        sift_settings.sift_base_url = "http://mock-sift.com"
        sift_settings.save(ignore_permissions=True)

        try:
            # Execute processing
            from frappe_cadence.cadence.multi_channel_cadence import process_cadence_step
            process_cadence_step(cadence.name, schedule.name)
            ...
            system_content = payload["input"][0]["content"][1]["text"]
            self.assertIn("Test Sender", system_content)
            self.assertIn("**Bold Bio**", system_content)
            self.assertIn("*italic*", system_content)

            user_content_list = payload["input"][1]["content"]
            history_text = user_content_list[1]["text"]
            self.assertIn("Previous History\n================", history_text)
            self.assertIn("Some notes", history_text)

        finally:
            # Cleanup single explicitly and delete docs to avoid orphans
            sift_settings.sift_base_url = original_url
            sift_settings.save(ignore_permissions=True)
            frappe.delete_doc("Multi Channel Cadence", cadence.name, force=True, ignore_permissions=True)
            frappe.delete_doc("Email Template", template.name, force=True, ignore_permissions=True)
            frappe.delete_doc("History", history.name, force=True, ignore_permissions=True)
